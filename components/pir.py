...
def motion_detected(publish_event, dht_settings, verbose=False):
    global publish_data_counter, publish_data_limit
...
def run_pir(settings, threads, stop_event):
    if settings['simulated']:
        ...
        pir_thread = threading.Thread(target=run_pir_simulator, args=(2, motion_detected, stop_event,  publish_event, settings))
        pir_thread.start()
        ...
        pir_thread = threading.Thread(target=run_pir_loop, args=(pir, motion_detected, stop_event, publish_event, settings))
        pir_thread.start()
        threads.append(pir_thread)
